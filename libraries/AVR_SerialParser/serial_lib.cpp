  ...
  if (serial_bus.available()) {
    ...
    switch (state) {
      ...
      case CRC: {
        ...
        memcpy(crc_buf, (uint8_t *)incoming_preamble, 3);  // put the preamble
                                                           // in
        ...
        if (data_bytes[0] >= SET_SERVO_OPEN_CLOSE &&
            data_bytes[0] < COMMAND_END)  // this is a valid command
        {
          msg.command = data_bytes[0];  // copy command into msg struct
          memcpy(&msg.data, &data_bytes[1],
                 length -
                     1);  // copy from command byte to end of msg into msg.data
          msg_parsed = true;
        }
